...
with a lag of `r l_opt-1` days (Pearson's correlation = `r lag_cor$r[l_opt]`; 95% CI: `r lag_cor$r_low[l_opt]` - `r lag_cor$r_hi[l_opt]`). 
...
Fitting a quasi-Poisson GLM, we found that over `r round(Rsq(lag_mod), 3)*100`% of the deviance in daily
...
before (intercept = `r round(exp(coefficients(lag_mod))[1])`, 95% CI: `r round(exp(confint(lag_mod))[1])` - `r round(exp(confint(lag_mod))[3])`; % increase per 1000 notifications =
`r round(coefficients(lag_mod)[2]*1000, 2)`, 95% CI: `r round(confint(lag_mod)[2]*100000, 2)` - `r round(confint(lag_mod)[4]*100000, 2)`)) (Figure \@ref(fig:reportsvsdeaths)).
...
```{r reportsvsdeaths, fig.cap="\\small \\textbf{Daily total COVID-19 deaths reported in England, against the number of potential COVID-19 cases reported through NHS Pathways with a lag of 16 days.} The black line and grey ribbon correspond to predictions from the regression model and associated 95\\% confidence intervals.", out.width = "70%", fig.align = "center"}
