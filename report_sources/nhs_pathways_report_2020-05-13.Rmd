...
using the approach described in Wallinga and Lipsitch [REF] and implemented in the
epitrix package [REF], with a serial interval modelled as a gamma distribution
with mean 4.7 days and standard deviation 2.9 days [REF].
...
All analyses were performed using the R software [REF], and the code is publicly
...
`r round(coefficients(latest_reg)[2]*1000, 2)`, 95% CI: `r round(confint(latest_reg)[2]*100000, 2)` - `r round(confint(latest_reg)[4]*100000, 2)`)) (Figure \@ref(fig:reportsvsdeaths)).
