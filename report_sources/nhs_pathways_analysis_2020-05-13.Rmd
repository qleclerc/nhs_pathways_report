...
glm_fit <- all_tot_pred %>% 
    filter(!is.na(note_lag)) %>%
  ggplot(aes(x = deaths, y = note_lag)) +
  geom_point() + 
  geom_line(aes(x = exp(pred))) + 
  geom_ribbon(aes(xmin = low, xmax = hi), alpha = 0.3, col = "grey") +
  theme_bw() +
  labs(x = "Daily number of deaths reported",
       y = "Daily number of \nNHS Pathways reports") +
  large_txt
